...
│   └── world-cache/              # build-time generator of countries.bin + world_meshes.bin
...
The device neither parses `world.geojson` nor triangulates the globe. Both happen
at build time: `tools/world-cache` is a small JVM program that compiles the app's
own `GeoJsonParser`, `PolygonTriangulator` and file formats a second time, for
the build machine, and `GenerateWorldCache` in `app/build.gradle.kts` runs it once
per variant and adds what it writes as generated assets:

| Asset | Holds | Compressed in the APK |
| --- | --- | --- |
| `countries.bin` (`CountriesFile`) | every `GeoJsonCountry`, coordinates as the parsed doubles | 1.3 MB |
| `world_meshes.bin` (`WorldMeshesFile`) | country fills, and borders as centerlines widened on load | 8.4 MB |

`world.geojson` itself no longer ships (`ignoreAssetsPatterns`), so the APK grows
by ~8.6 MB net — iOS ships a 17.5 MB `globe.scn` for the same reason. Nothing is
checked in and nothing is regenerated by hand: the task reruns whenever the
GeoJSON or any code compiled into the tool changes. That code is an explicit list
of files in `tools/world-cache/build.gradle.kts`; if the parser or triangulator
starts using another file, add it there — the build fails until it is.

Measured on a Galaxy A55, release build, cold launches (both log their timing,
under the `CountryDataCache` and `GlobeGeometryCache` tags):

| Step | Parsed / triangulated on device | Loaded from the cache |
| --- | --- | --- |
| Countries | ~600 ms | ~14 ms |
| Globe geometry, after the countries | ~1,240 ms | ~60 ms |

`CountriesFileTest` and `WorldMeshesFileTest` hold both files to what parsing and
triangulating produce, bit for bit, which is what lets every parser and
triangulator test keep speaking for what the app actually loads. JVM tests never
read the generated files themselves: `SharedFiles.countryDataCache()` parses the
GeoJSON, and `GlobeGeometryCache` triangulates until `install` points it at the
asset.
...
globe's geometry — both loaded from the build's [world caches](#world-caches) —
and both are prewarmed off the main thread from `VoyageApplication.onCreate`. `EarthTextureCache` holds the decoded
